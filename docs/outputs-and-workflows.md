...
| **`graph-viewer.html`** | HTML | A self-contained, **fully offline** interactive viewer (fonts embedded, no CDN, no network), in light or dark to match the system. Nodes coloured by kind, edges by type; click a node to read its summary and walk its edges, click an edge to read why it exists, and open **About this graph** for a guide written from the graph's own counts. Generated by `build_graph_viewer.py`. |
